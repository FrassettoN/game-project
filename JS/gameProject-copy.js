...
import { GAME_LEVELS } from './gameLevels-copy.js';
...
State.prototype.update = function (time, keys, directions) {
  // keys = keys that are being held down
  let actors = this.actors.map((actor) =>
    actor.update(time, this, keys, directions)
  );
...
function runLevel(level, Display) {
  ...
  return new Promise((resolve) => {
    ...
    let touchDirections = trackDirections();
    ...
    function frame(time) {
      ...
      state = state.update(time, arrowKeys, touchDirections);
